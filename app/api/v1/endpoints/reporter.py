...
async def get_report_status(report_id: str) -> ReportDetailsResponse:
    ...
    for metadata in report_data.get("metadata", []):
        ...
        download_urls[format_name] = f"/api/v1/reporter/download/{report_id}/{format_name}"
    
    response = ReportDetailsResponse(
        report_id=report_id,
        status=report_data["status"],
        metadata=report_data.get("metadata", []),
        summary=report_data.get("summary", ReportSummary(
            execution_summary={},
            test_coverage={},
            performance_metrics={},
            failure_analysis={},
