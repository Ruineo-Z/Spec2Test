...
async def get_execution_status(execution_id: str) -> ExecutionResultResponse:
    ...
    response = ExecutionResultResponse(
        ...
        summary=execution_data.get("summary", ExecutionSummary(
            total_tests=0,
            passed=0,
            failed=0,
            skipped=0,
...
async def cancel_execution(execution_id: str) -> Dict[str, Any]:
    ...
    if execution_data["status"] in [ExecutionStatus.COMPLETED, ExecutionStatus.FAILED, ExecutionStatus.CANCELLED]:
...
async def run_tests_background(
    ...
    try:
        ...
        execution_store[execution_id].update({
            ...
            "artifacts": [
                {"name": "test_report.html", "path": "/reports/test_report.html", "type": "html"},
                {"name": "test_results.json", "path": "/reports/test_results.json", "type": "json"}
            ]
        })
