...
class TestSettings(BaseSettings):
    ...
    # 测试用例生成配置
    max_test_cases_per_endpoint: int = Field(default=10, env="TEST_MAX_CASES_PER_ENDPOINT")
    ...
    # 报告配置
    report_formats: List[str] = Field(default=["html", "json"], env="TEST_REPORT_FORMATS")
...
def validate_settings() -> bool:
    ...
    try:
        ...
        # 验证目录权限
        assert settings.work_dir.exists(), f"Work directory {settings.work_dir} does not exist"
        assert settings.temp_dir.exists(), f"Temp directory {settings.temp_dir} does not exist"
