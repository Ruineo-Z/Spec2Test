...
from app.core.models import APIEndpoint, DocumentAnalysis, HttpMethod, DocumentQuality
...
class TestOpenAPIParser:
    ...
    def test_extract_endpoints_contains_expected_paths(self, parser, sample_openapi_file):
        """测试提取的端点包含预期的路径"""
        spec = parser.parse_file(sample_openapi_file)
        endpoints = parser.extract_endpoints(spec)
    ...
    def test_extract_endpoints_with_parameters(self, parser, sample_openapi_file):
        ...
        get_pets_endpoint = next(
            (ep for ep in endpoints if ep.path == '/pets' and ep.method == HttpMethod.GET),
            None
        )
    ...
    def test_extract_endpoints_with_path_parameters(self, parser, sample_openapi_file):
        ...
        get_pet_endpoint = next(
            (ep for ep in endpoints if ep.path == '/pets/{petId}' and ep.method == HttpMethod.GET),
            None
        )
    ...
    def test_extract_endpoints_with_request_body(self, parser, sample_openapi_file):
        ...
        post_pets_endpoint = next(
            (ep for ep in endpoints if ep.path == '/pets' and ep.method == HttpMethod.POST),
            None
        )
    ...
    def test_extract_endpoints_with_responses(self, parser, sample_openapi_file):
        ...
        for endpoint in endpoints:
            ...
            assert '200' in endpoint.responses or '201' in endpoint.responses or '204' in endpoint.responses
    ...
    def test_analyze_quality_good_document(self, parser, sample_openapi_file):
        ...
        assert analysis.quality_level in [DocumentQuality.GOOD, DocumentQuality.EXCELLENT]
